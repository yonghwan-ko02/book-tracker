...
### F1. 하이브리드 검색 및 3중 안심 도서 검색 파이프라인
* **요구사항**: 사용자 검색 시 무중단 실시간 정보 렌더링.
* **상세**:
  - **1단계**: Google Books OpenAPI를 실시간 호출.
  - **2단계 (CORS-Open 대체)**: Google API 호출 실패(429 과도한 트래픽 제한) 시, 즉시 **Open Library OpenAPI**로 실시간 우회 호출.
  - **3단계 (오프라인 백업)**: 인터넷 차단 또는 두 API 서버 마비 시, 기 탑재된 11종의 대표 베스트셀러 내장 데이터베이스(`MOCK_BOOKS`)에서 로컬 필터 검색 제공.
...
* **상세**:
  ...
  - 클릭 시 장바구니 내 책의 ISBN(또는 제목)을 조합하여 **교보문고 통합검색** 및 **알라딘 통합검색** 탭을 `window.open` 브라우저 멀티 탭으로 동시 연결.
